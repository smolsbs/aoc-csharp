...
public abstract class Problem<TPart1, TPart2> : IProblem
{
	...
	protected string GetInputFilePath(string filename = "input.txt")
	{
		var info = this.GetType().GetCustomAttribute<ProblemInfoAttribute>();
		if (info == null)
			return filename;

		return Path.Combine($"Years/year_{info.Year}/day{info.Day:00}", filename);
	}
	...
	{
		return File.ReadAllText(GetInputFilePath(filename));
	}
}